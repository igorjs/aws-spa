...
yargs
  ...
  .command(
    "deploy-lambda-edge <domainName> <codePath>",
    "Attach a Lambda Edge on AWS to CloudFront domain",
    (yargs) => {
      return yargs
        .positional("domainName", {
          type: "string",
          demand: true,
          describe: `The domain name on which the Lambda will be attached. For example "app.example.com".
        ...
        .positional("codePath", {
          type: "string",
          demand: true,
          describe: `The path of the lambda code in your app source code.`,
        });
    },
    async (argv) => {
      if (!argv.domainName) {
